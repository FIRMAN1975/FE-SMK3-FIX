...
function PortofolioDetail() {
  ...
  const handleSubmit = async () => {
    ...
    if (res.success) setActionMsg('✅ Portofolio berhasil diajukan untuk review!');
    else setActionMsg('❌ Gagal mengajukan: ' + res.message);
  };
  ...
  const handlePublish = async () => {
    ...
    if (res.success) setActionMsg('✅ Portofolio berhasil dipublish!');
    else setActionMsg('❌ Gagal publish: ' + res.message);
  };
  ...
  const handleReject = async () => {
    ...
    if (res.success) {
      setActionMsg('✅ Portofolio berhasil ditolak.');
      setShowRejectModal(false);
      setRejectReason('');
    } else {
      setActionMsg('❌ Gagal menolak: ' + res.message);
    }
  };
      ...
      <div style={{ flex: 1, padding: '40px 52px' }}>
        ...
        {actionMsg && (
          <div style={{
            ...
            border: `1px solid ${actionMsg.startsWith('✅') ? '#bbf7d0' : '#fca5a5'}`,
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: '24px' }}>
          <div>
            <div style={{ backgroundColor: 'white', borderRadius: '12px', padding: '24px', marginBottom: '20px', boxShadow: '0 1px 4px rgba(0,0,0,0.07)', minHeight: '260px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {portfolio.image ? (
                <img src={portfolio.image} alt={portfolio.title} style={{ maxWidth: '100%', maxHeight: '360px', borderRadius: '8px', objectFit: 'cover' }} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
              ) : (
