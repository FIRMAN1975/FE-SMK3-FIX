      ...
      <div style={{ flex: 1, padding: '40px 52px' }}>
        ...
        {isAuth && (
          <div style={{
            ...
            ✅ Login sebagai <strong>{namaUser || 'Pengguna'}</strong> — kamu hanya bisa mengedit karya milikmu sendiri.
          </div>
        )}
        ...
        <div style={{ backgroundColor: 'white', borderRadius: '12px', boxShadow: '0 1px 6px rgba(0,0,0,0.07)', overflow: 'hidden' }}>
          ...
          {loading ? (
            ...
            <EmptyState icon="📂" message={search || filterJurusan ? 'Tidak ada hasil yang cocok' : 'Belum ada data portofolio'} />
          ) : (
            paginated.map((p, i) => (
              <div
                key={p.id}
