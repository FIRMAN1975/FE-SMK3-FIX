...
export default function AdminPelanggaranPage() {
  ...
  const handleDownloadPDF = (id) => {
    // Membuka PDF wajib pakai URL lengkap Gateway + Prefix
    const baseUrl = apiGateway.defaults.baseURL;
    window.open(`${baseUrl}${PREFIX}/${id}/pdf`, "_blank");
  };
  ...
  return (
    <AdminLayout title="Pelanggaran & Surat Panggilan">
      <AdminCard 
        title="⚠️ Daftar Surat Panggilan" 
        ...
        <div className="smk-form-group">
          ...
          {showGuruDropdown && (
            <div style={{ border: '1px solid #e5e9f2', borderTop: 'none', borderRadius: '0 0 8px 8px', padding: '12px', maxHeight: '160px', overflowY: 'auto', background: '#f8faff', marginTop: '-4px' }}>
              {masterGuru.length === 0 ? (
                <div style={{ fontSize: '12px', color: '#ef4444', textAlign: 'center' }}>⚠️ Gagal memuat data guru.</div>
              ) : (
                masterGuru.map(guru => (
